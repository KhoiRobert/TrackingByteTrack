...
def predict_new_detection(stracks, img, test_size):
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if is_time_record:
                    start = time.time()
                # if (frame_id-1) % 2 != 0:
                #     outputs = predict_new_detection(online_targets,imgs[0],test_size)
                # else:
                #     outputs = model(imgs)
            ...
            if outputs[0] is not None:
                online_targets = tracker.update(outputs[0], info_imgs, self.img_size, frame_id-1)
                online_tlwhs = []
                online_ids = []
                online_scores = []
