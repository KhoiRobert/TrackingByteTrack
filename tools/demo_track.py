...
def predict_new_detection(stracks, img, predictor):
    outputs = STrack.persist_multi_predict(stracks)
    outputs = outputs[:, :4]
    r = min(predictor.test_size[0] / img.shape[0], predictor.test_size[1] / img.shape[1])
    outputs *= r
    center = outputs[:,:2].copy()
    outputs[:,0] = outputs[:,0] - (outputs[:,2]*outputs[:,3]) / 2
...
def  imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            ...
            if frame_id % 2 != 0:
              outputs, img_info = predict_new_detection(online_targets, frame, predictor)
            else:
                outputs, img_info = predictor.inference(frame, timer)
            # outputs, img_info = predictor.inference(frame, timer)
            
            dec_end = time.perf_counter()
            print("Dectection: ", dec_end - dec_start)
            if outputs[0] is not None:
